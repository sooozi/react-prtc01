...
import { clearGlobalApiError, setGlobalApiErrorText } from "@/api/http/apiErrorDisplay";

function WithGlobalApiError({ message, children }: { message: string; children: ReactNode }) {
  useEffect(() => {
...
export const LongMessage: Story = {
  decorators: [
    (Story) => (
      <WithGlobalApiError message="API 응답을 받지 못했습니다. 서버 상태를 확인한 뒤 다시 시도해주세요. 문제가 계속되면 관리자에게 문의하세요.">
        <Story />
      </WithGlobalApiError>
    ),
  ],
