...
| 변수 | 설명 |
|------|------|
| `VITE_API_BASE_URL` | API 베이스 URL. 로그인·회원가입·게시판 등 실서버 연동 요청에 사용. 미설정 시 연동 API가 동작하지 않습니다. 사용자 목록은 mock만 사용할 수 있습니다. 실제 값은 `.env`에만 두고 저장소에는 올리지 않습니다. |
...
`/home`, `/auth/*`, `/about`, `/forbidden`은 비로그인 접근 가능하고, `/post/*`, `/user/search`, `/user/mypage` 등은 `RequireAuth`로 토큰 검사 후 없으면 `/auth/login`으로 보냅니다.  
...
- **목록 (`/post/list`)**: 로그인 필수. 목록 조회 + 페이지네이션 + 정렬. URL 쿼리 `page`, `sort`(정렬 기준), `order`(ASC/DESC)는 `useUrlQueryPage` 등으로 동기화하며, 정렬은 API의 `sortColumnName`, `sortType`으로 전달. 검색 조건은 URL이 아니라 화면 state로만 유지. 정렬 옵션: 최신순(regDt), 제목순(title), 조회순(inqCnt). `page` 유효성 보정(음수·초과 시 1 또는 마지막 페이지). 로딩/에러/빈 목록 처리.
...
- **주요 경로 예시**:  
  ...
  사용자 목록은 현재 백엔드 연동 없이 `mocks` 데이터를 쓰며, 연동 시 `src/api/user/userApi.ts`를 수정하면 됩니다.
...
백엔드가 위 주소 규격으로 동작해야 로그인·회원가입·게시판 연동이 정상이며, 사용자 목록은 mock 기준으로 동작합니다.
...
- **컴포넌트 export**: `src/components/index.ts`에서 Badge, Button, Confirm, LoadingState, Header, Footer, Layout, Pagination, Tooltip export. (`CatHover` 등은 직접 경로 import)
