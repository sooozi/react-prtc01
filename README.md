...
| 항목                                      | 상태                                                                                                  |
| --------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `pages/schedule/components/work-time-bar/WorkTimeBar.tsx` | **존재하나 현재 `Schedule.tsx`에서 import 되지 않음** (예비·실험 컴포넌트) |
...
`ApiErrorBar`, `Badge`, `Button`, `Confirm`, `Footer`, `Header`, `Icons`, `ImageFileAttachField`, `LoadingState`, `PageHeader`, `Pagination`, `PostHtmlContent`, `RichTextEditor`, `TableSortHeader`, `Tooltip` 등.
...
│   └── schedule/      # calendarUtils, workTimeUtils
├── mocks/             # 목·데모 데이터
│   ├── user.ts
...
│           └── work-time-bar/  # WorkTimeBar (현재 Schedule 미연결)
├── router/
├── styles/
├── utils/
├── bootstrapAxe.ts
